...
public class Laplace{
	...
	while(true){
	System.out.print("Give lambda:");
	lambda = Inp.nextDouble();
	makeExp(lambda);
	filterLaplace();
    ...
    public static void filterLaplace(){
	int i, j, k, l, n;
    ...
    } // end method filterLaplace

    public static void makeExp(double lambda){
	int n;
	double s;

	for(n=0; n<=n_max; n++){
	    s = Math.sqrt((double)n);
	    arrExp[n] = Math.exp(-lambda * s);
	}
